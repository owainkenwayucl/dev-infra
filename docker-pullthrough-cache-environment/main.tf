...
resource "harvester_virtualmachine" "vm" {
  ...
  name                 = "${var.username}-dockercache-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = var.namespace
  ...
  hostname        = "${var.username}-dockercache-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
