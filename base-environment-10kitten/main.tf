...
resource "harvester_virtualmachine" "vm" {
  ...
  name                 = "${var.username}-base-10kitten-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = var.namespace
  ...
  hostname        = "${var.username}-base-10kitten-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
