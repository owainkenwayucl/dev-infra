...
resource "harvester_virtualmachine" "vm" {
  ...
  name                 = "${var.username}-grafana-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = var.namespace
  ...
  hostname        = "${var.username}-grafana-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  ...
  tags = {
    ...
    condenser_ingress_reg_hostname = "${var.username}graf"
    condenser_ingress_reg_port = 3000
