...
resource "harvester_virtualmachine" "servicevm" {
  
  count = 1

  name                 = "${var.username}-service-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = var.namespace
  ...
  hostname        = "${var.username}-service-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
